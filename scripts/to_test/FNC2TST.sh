...
### Functions
genScript() {
    echo "#!/usr/bin/env bash" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "installmtd=\"${installmtd}\"" >> archBaseInstall2.sh
    echo "usrkymp=\"${usrkymp}\"" >> archBaseInstall2.sh
    echo "default_title=\"${default_title}\"" >> archBaseInstall2.sh
    echo "steps=\"${steps}\"" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "cliInstall() {" >> archBaseInstall2.sh
    ...
    echo "}" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "ncursesAutoInstall2() {" >> archBaseInstall2.sh
    echo "    sleep 5" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 7/\$steps\" --msgbox \"\nUpdating the ZoneInfo to America/Sao_Paulo\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    ln -sf /usr/share/zoneinfo/America/Sao_Paulo /etc/localtime" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 8/\$steps\" --msgbox \"\nSyncronizing the hardware clock to the system clock\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    hwclock --systohc" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 9/\$steps\" --msgbox \"\nEditing /etc/locale.gen to pt_BR.UTF-8\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    sed -i \"s/#pt_BR.UTF-8 UTF-8/pt_BR.UTF-8 UTF-8/\" /etc/locale.gen" >> archBaseInstall2.sh
    echo "    locale-gen" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 10/\$steps\" --msgbox \"\nSaving the locale in /etc/locale.conf\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    if [ -e /etc/locale.conf ]; then" >> archBaseInstall2.sh
    echo "        echo \"LANG=pt_BR.UTF-8\" >> /etc/locale.conf" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        touch /etc/locale.conf" >> archBaseInstall2.sh
    echo "        echo \"LANG=pt_BR.UTF-8\" >> /etc/locale.conf" >> archBaseInstall2.sh
    echo "    fi" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 11/\$steps\" --msgbox \"\nSaving the keyboard layout in /etc/vconsole.conf\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    if [ -e /etc/vconsole.conf ]; then" >> archBaseInstall2.sh
    echo "        echo \"KEYMAP=br-abnt2\" >> /etc/vconsole.conf" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        touch /etc/vconsole.conf" >> archBaseInstall2.sh
    ...
    echo "    fi" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    hstnm=\$(dialog --title \"\$default_title 12/\$steps\" --output-fd 1 --inputbox \"Enter this machine hostname: \" 25 90); clear" >> archBaseInstall2.sh
    echo "    if [[ -e /etc/hostname ]]; then" >> archBaseInstall2.sh
    echo "        echo \$hstnm >> /etc/hostname" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        touch /etc/hostname" >> archBaseInstall2.sh
    ...
    echo "    fi" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    dialog --title \"\$default_title 13/\$steps\" --msgbox \"\nGenerating the hosts file\" 25 90 && clear" >> archBaseInstall2.sh
    echo "    echo \"# =====================================\" >> /etc/hosts" >> archBaseInstall2.sh
    echo "    echo \"# IPv4	Config\" >> /etc/hosts" >> archBaseInstall2.sh
    echo "    echo \"127.0.0.1	localhost\" >> /etc/hosts" >> archBaseInstall2.sh
    echo "    echo \"::1		localhost\" >> /etc/hosts" >> archBaseInstall2.sh
    ...
    echo "    echo -e \"\$rtpass\n\$rtpass\" | sudo passwd -q" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    if dialog --title \"\$default_title 15/\$steps\" --yes-label \"Intel\" --no-label \"AMD\" --yesno \"Is your processor Intel or AMD?\" 7 64; then" >> archBaseInstall2.sh
    echo "        pacman -S intel-ucode" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        pacman -S amd-ucode" >> archBaseInstall2.sh
    echo "    fi" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    instpkgs=\$(dialog --title \"\$default_title 16/\$steps\" --output-fd 1 --inputbox \"Type \\"AUTO\\" to install the packages from the auto installation\nAUTO=grub networkmanager dialog wireless_tools wpa_supplicant os-prober mtools dosfstools base-devel linux-headers iwd dhcpcd\nEnter the packages that you want to install:\" 25 75); clear" >> archBaseInstall2.sh
    echo "    lwcs_instpkgs=\$(echo \"\$instpkgs\" | tr '[:upper:]' '[:lower:]')" >> archBaseInstall2.sh
    echo "    if [[ \"\$lwcs_instpkgs\" == \"auto\" ]]; then" >> archBaseInstall2.sh
    echo "        pacman -S grub networkmanager dialog wireless_tools wpa_supplicant os-prober mtools dosfstools base-devel linux-headers iwd dhcpcd" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        pacman -S \"\$instpkgs\"" >> archBaseInstall2.sh
    echo "    fi" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "    if [[ -e \$(command -v grub) ]]; then" >> archBaseInstall2.sh
    echo "        tgt_plat=\$(dialog --title \"\$default_title 17/\$steps\" --no-tags --output-fd 1 --menu \"Select your desired platform to install GRUB: \" 7 60 0 \"arm-coreboot\" \"arm-coreboot\" \"arm-efi\" \"arm-efi\" \"arm-uboot\" \"arm-uboot\" \"arm64-efi\" \"arm64-efi\" \"i386-coreboot\" \"i386-coreboot\" \"i386-efi\" \"i386-efi\" \"i386-ieee1275\" \"i386-ieee1275\" \"i386-multiboot\" \"i386-multiboot\" \"i386-pc\" \"i386-pc\" \"i386-qemu\" \"i386-qemu\" \"i386-xen\" \"i386-xen\" \"i386-xen_pvh\" \"i386-xen_pvh\" \"ia64-efi\" \"ia64-efi\" \"mips-arc\" \"mips-arc\" \"mips-qemu_mips\" \"mips-qemu_mips\" \"mipsel-arc\" \"mipsel-arc\" \"mipsel-loongson\" \"mipsel-loongson\" \"mipsel-qemu_mips\" \"mipsel-qemu_mips\" \"powerpc-ieee1275\" \"powerpc-ieee1275\" \"riscv32-efi\" \"riscv32-efi\" \"riscv64-efi\" \"riscv64-efi\" \"sparc64-ieee1275\" \"sparc64-ieee1275\" \"x86_64-efi\" \"x86_64-efi\" \"x86_64-xen\" \"x86_64-xen\"); clear" >> archBaseInstall2.sh
    echo "        _grb_dsk=\$(dialog --title \"\$default_title 17/\$steps\" --output-fd 1 --inputbox \"Enter the name of the disk to install GRUB:\" 25 75); clear" >> archBaseInstall2.sh
    echo "        if [[ \"\$_grb_dsk\" =~ .*\"/dev/\".* ]]; then" >> archBaseInstall2.sh
    echo "            grb_dsk=\$(echo \"\$_grb_dsk\" | cut -d/ -f3-)" >> archBaseInstall2.sh
    echo "        else" >> archBaseInstall2.sh
    echo "            grb_dsk=\$_grb_dsk" >> archBaseInstall2.sh
    echo "        fi" >> archBaseInstall2.sh
    echo "        grub-install -v --target=\$tgt_plat /dev/\$grb_dsk" >> archBaseInstall2.sh
    echo "        grub-mkconfig -o /boot/grub/grub.cfg" >> archBaseInstall2.sh
    echo "    else" >> archBaseInstall2.sh
    echo "        othr_btmgr=\$(dialog --title \"\$default_title 17/\$steps\" --output-fd 1 --inputbox \"Please provide the command to install your custom bootloader:\" 25 75); clear" >> archBaseInstall2.sh
    echo "        \$othr_btmgr" >> archBaseInstall2.sh
    echo "    fi" >> archBaseInstall2.sh
    echo "}" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "# Checks if dialog exists, if not go with default cli installation" >> archBaseInstall2.sh
    echo "if [[ ! -e \$(command -v dialog) || ! -f /usr/bin/dialog || ! -f /bin/dialog ]]; then" >> archBaseInstall2.sh
    echo "    printf \"%s\n\" \"dialog not found, proceeding with default cli installation\"" >> archBaseInstall2.sh
    echo "    cliInstall" >> archBaseInstall2.sh
    echo "fi" >> archBaseInstall2.sh
    echo "" >> archBaseInstall2.sh
    echo "# Checks the method used in the first script" >> archBaseInstall2.sh
    echo "if [[ \"\$installmtd\" == \"auto\" ]]; then" >> archBaseInstall2.sh
    echo "    ncursesAutoInstall2" >> archBaseInstall2.sh
    echo "elif [[ \"\$installmtd\" == \"manu\" ]]; then" >> archBaseInstall2.sh
    echo "    ncursesManualInstall2" >> archBaseInstall2.sh
    echo "else" >> archBaseInstall2.sh
    echo "    if [[ -e \$(command -v dialog) || -f /usr/bin/dialog || -f /bin/dialog ]]; then" >> archBaseInstall2.sh
    echo "        if dialog --title \"\$default_title\" --yes-label \"Automated\" --no-label \"Manual\" --yesno \"\nPrevious install method not found\nDo you want the Automated Install or the Manual Install?\" 7 64; then" >> archBaseInstall2.sh
    echo "            clear" >> archBaseInstall2.sh
    echo "            ncursesAutoInstall2" >> archBaseInstall2.sh
    echo "        else" >> archBaseInstall2.sh
    echo "            clear" >> archBaseInstall2.sh
